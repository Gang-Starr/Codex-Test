...
function buildManagementSummary(rows) {
  ...
  if (currentLanguage === 'it') {
    ...
    const oeeText = withOee ? ` ${formatInteger(withOee)} voci contengono dati OEE validi. ${formatInteger(withoutOee)} voci non hanno dati OEE completi. L’OEE medio è ${formatPercent(totals.oee)}.` : '';
    const oeeWarningText = withOee && warnings ? ` Nota: ${formatInteger(warnings)} valore/i OEE sono superiori al 100 % e devono essere controllati.` : '';
    return `In totale sono stati raggiunti ${formatInteger(totals.good)} pezzi buoni con ${formatInteger(totals.scrap)} pezzi di scarto. Il raggiungimento target medio è ${formatPercent(totals.achievement)}. ${formatInteger(redTargets)} voci sono sotto il 90 % di raggiungimento target.${targetGapText}${oeeText} Il giorno migliore è ${formatDate(bestDay.date)} con ${formatInteger(bestDay.good)} pezzi buoni.${oeeWarningText}`;
  }
  if (currentLanguage === 'en') {
    ...
    const oeeText = withOee ? ` ${formatInteger(withOee)} entries contain valid OEE data. ${formatInteger(withoutOee)} entries do not have complete OEE data. The average OEE is ${formatPercent(totals.oee)}.` : '';
    const oeeWarningText = withOee && warnings ? ` Note: ${formatInteger(warnings)} OEE value(s) are above 100 % and should be checked.` : '';
    return `A total of ${formatInteger(totals.good)} good parts were achieved with ${formatInteger(totals.scrap)} scrap parts. The average target achievement is ${formatPercent(totals.achievement)}. ${formatInteger(redTargets)} entries are below 90 % target achievement.${targetGapText}${oeeText} The best day is ${formatDate(bestDay.date)} with ${formatInteger(bestDay.good)} good parts.${oeeWarningText}`;
  }
  ...
  const oeeText = withOee ? ` ${formatInteger(withOee)} Einträge enthalten gültige OEE-Daten. ${formatInteger(withoutOee)} Einträge liegen ohne vollständige OEE-Daten vor. Die durchschnittliche OEE liegt bei ${formatPercent(totals.oee)}.` : '';
  const oeeWarningText = withOee && warnings ? ` Hinweis: ${formatInteger(warnings)} OEE-Wert(e) liegen über 100 % und sollten geprüft werden.` : '';
  return `Insgesamt wurden ${formatInteger(totals.good)} Gutteile bei ${formatInteger(totals.scrap)} Ausschussteilen erreicht. Die durchschnittliche Zielerreichung liegt bei ${formatPercent(totals.achievement)}. ${formatInteger(redTargets)} Einträge liegen unter 90 % Zielerreichung.${targetGapText}${oeeText} Stärkster Tag ist ${formatDate(bestDay.date)} mit ${formatInteger(bestDay.good)} Gutteilen.${oeeWarningText}`;
}
